                                          ...
                                          GError      **error)
{
  int repo_dfd = ostree_repo_get_dfd (repo);
  g_autofree char *path = NULL;
  g_autofree char *filename = g_strconcat (from_digest, "-", to_digest, NULL);
  struct stat stbuf;
                       ...
                       GError      **error)
{
  ...
  cached = g_hash_table_lookup (digested_summaries, digest);
  if (cached)
  ...
  g_hash_table_insert (digested_summaries, g_strdup (digest), g_variant_ref (loaded));
                ...
                GError         **error)
{
  ...
  old_content = read_digested_summary (repo, old_digest, digested_summaries, cancellable, NULL);
  if  (old_content == NULL)
  ...
  for (l = ordered_summaries; l; l = l->next)
    {
      ...
      subsummary_content = read_digested_summary (repo, digest, digested_summaries, cancellable, error);
          ...
          if (var_summary_index_subsummaries_lookup (old_subsummaries, subsummary, NULL, &old_subsummary))
            {
              ...
              if (!add_to_history (repo, history_builder, parent, digest_v, subsummary_content, digested_summaries,
              ...
              for (gsize i = 0; i < len; i++)
                {
                  ...
                  if (!add_to_history (repo, history_builder, c, digest_v, subsummary_content, digested_summaries,
                                       &history_len, max_history_length, cancellable, error))
                    return FALSE;
                }
            }
  ...
  if (!disable_index)
    {
      ...
      summary_index = generate_summary_index (repo, old_index, summaries, digested_summaries,
